...
**【演讲脚本】**
大家好，感谢大家参加今天的MonthlyShare。
今天我想介绍一个很热的技术——**Agent Skills**。随着 AI Agent 在业务中的应用越来越深，我们发现单纯靠 Prompt Engineering 已经很难维护复杂的业务逻辑。Agent Skills 提供了一套标准化的解决方案，让我们可以像管理代码模块一样管理 Agent 的能力。
接下来让我开始。
...
**【演讲脚本】**
接下来进入具体的技术细节。Agent Skills 的核心架构设计理念叫做 **Progressive Disclosure（渐进式披露）**。
...
**【演讲脚本】**
写 Skill 的过程，我们称为 **Context Engineering**。这里有几条要遵守的规范：
...
第三是 **清晰的输入输出格式**。Agent 依赖我们定义的 Input 和 Output 格式。如果格式不清晰，Agent 很容易出错。
